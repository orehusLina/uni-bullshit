// Создать список, содержащий целые числа. После всех чисел, равных минимальному числу, вставить последний нечетный элемент. 
// Например, было  2 8 2 1 6 8 8 1 2 2 8 2 1 3. Стало  2 8 2 1 3 6 8 8 1 3 2 2 8 2 1 3 3 (n = 14)
#include <iostream>
using namespace std;
...
void result(list *&h, list *&t) {  // функция для изменения списка в соответствии с заданием
    ...
    while (p) { // пока не дошли до конца списка
        ...
        if (p->inf == minim) // если элемент минимальный
            insert_after(h, t, p, odd); //вставляем новый
        p = k; // присваиваем p следующий элемент
    }
}
